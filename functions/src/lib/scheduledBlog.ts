  ...
  for (const doc of snapshot.docs) {
    ...
    try {
      const imageRes = await fetch(
        `https://api.unsplash.com/photos/random?query=${encodeURIComponent(productName)}&client_id=${UNSPLASH_ACCESS_KEY}`
      );
      const imageJson = await imageRes.json();
      imageUrl = imageJson?.urls?.regular ?? "";
    } catch {
      console.warn(`⚠️ Unsplash画像取得に失敗: ${productName}`);
    }
