// functions/src/utils/extractFeatureHighlights.ts
 ...
 * @return {Promise<string[]>} 要点3つの配列（例：["保温性", "掃除のしやすさ", "通気性"]）
 */
export async function extractFeatureHighlights(features: string): Promise<string[]> {
  const prompt = `
以下の爬虫類用品の特徴説明から、重要なポイントを3つに要約してください。
できるだけ端的かつ、飼育者が比較しやすいキーワード（例：保温性、掃除のしやすさ、通気性）で返答してください。
改行区切りの箇条書きで出力してください。
